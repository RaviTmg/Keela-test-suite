	...
	it('Add Volunteering Activity', () => {
		...
		cy.fixture('user').then(user => {
			...
			cy.get('#main-container')
				.get('.content-scroll-header')
				.get('.btn')
				.contains('Add Contact')
				.click()
			cy.get(
				'[data-tracking-input="contacts-PersonContacts-firstName"]'
			).within(() => {
			...
			cy.get('#main-container')
				...
				.click()
			cy.get(
				'[data-tracking-input="contact-volunteering-VolunteerRecords-name"]'
			).within(() => {
				cy.get('input').type(activityName)
			})

			cy.get(
				'[data-tracking-input="contact-volunteering-VolunteerRecords-description"]'
			).within(() => {
				cy.get('textarea').type(description)
			})

			cy.get('#modals-container')
				.get('.block-content')
				.get('.btn')
				.contains('Save')
				.click()

			cy.get('.notification-title', { timeout: 10000 }).contains(' Volunteering Activity Successfully added', { matchCase: false })
